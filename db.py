...
def create_db_tables(conx):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS resource(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, author TEXT, isbn_10 TEXT, isbn_13 TEXT, date_added TEXT DEFAULT CURRENT_DATE)")
...
def token_is_valid(session_id, token):
    ...
    # Has the TTL expired?
    token_created_dt = datetime.strptime(time_created, "YYYY-MM-DD HH:MM:SS.SSS")
...
def add_resource(title, author, isbn10, isbn13):
    conx = get_sqlite3_conx(DB_NAME)
    resource_id = update_db(
        conx,
        "INSERT INTO resource(title, author, isbn_10, isbn_13) VALUES(?, ?, ?, ?)",
...
def add_resource_to_inventory(title, author, isbn10, isbn13):
    conx = get_sqlite3_conx(DB_NAME)
    ...
    if existing_resource:
        ...
        resource_id = add_resource(title, author, isbn10, isbn13)
    return resource_id
